...
Vektor je v jazyce R základní stavební strukturou, nic jaká skalární veličina zde není. Koncept vektoru je zde chápán v matematickém smyslu jako vektor hodnot, reprezentující bod v $n-$dimenzionálním prostoru. Může nabývat jakéhokoliv datového typu, nicméně všechny prvky v daném vektoru jsou právě jednoho typu, čímž rozumíme, že je tato struktura tzv. homogenní. Vektor je možné vytvořit mnoha způsoby, mezi nejčastější patří funkce `vector(mode = "numeric", length = 10)` a funkce `c()`, případně vzniká pomocí opetárorů `[` nebo `[[`.
...
Zajišťují obecný průběh programu, tvoří základní prvky algoritmů. Mezi řídící 
strukury řadíme podmínky a cykly. Mohli bychom přidat i složené závorky,
něboť pro struktury uvozují bloky kódu.
...
Pokud je předem znám počet opakování cyklu (nebo je i jinak implicitně určen),
používá se `for` cyklu.
...
V případech, kdy počet opakování není předem znám, ale je známa podmínka, za které
cyklus má probíhat.
...
Kromě únikového klíčového slova `break` po němž cyklus okamžitě skončí je k využití 
příkaz `next`, po němž se zbytek příkazů v dané iteraci přeskočí a operátor `%||%`.

::: callout-tip
## Úloha
  
1. Vytvořte cyklus, který 
:::
