...
1.  Vyhledejte nápovědu k `DateTimeClasses`.\
    a)  Co reprezentují třídy `POSIXct` a `POSIXlt`? Jaký je mezi nimi rozdíl?\
    b)  V nápovědě k funkci `min` najděte a spustťte příklady užití.\
...
2.  Intuitivně by nás mohlo vést nahrát data do proměnné `data`. Je to chybný postup, neboť `data()` je funkce zpřístupňující různé datové sady, například ty, které jsou běžnou součástí instalace (v balíčku `datasets`). Vyzkoušejte s pomocí nápovědy `library(help = datasets)`.
:::
...
4.  Pomocí funkce `cat()` se pokuste vypsat na konzoli tento text:  Říkám: "Mám rád R!".
:::
...
5.  Použite k výpočtu operátory či množinové funkce:\
...
6.  Vyhodnoťte s pomocí R následující výrazy:\
    ...
    i)  $\mathrm{diag}\left( \pmatrix{5\\ 3}\cdot \pmatrix{6 & 5}\right)=$ `r fitb(c("30"))`\
    j)  $y=x^2 + \sqrt[3]{x};\:y^\prime=$ `r fitb("2 * x + x^((1/3) - 1) * (1/3)")`
:::
...
Vektor je v jazyce R základní stavební strukturou, nic jako skalární veličina zde není. 
<!-- Koncept vektoru je zde chápán v matematickém smyslu jako jednosloupcová matice $\boldsymbol{\mathrm{A}} = (a_{ij})_{m1}$ hodnot. -->

![Vektor je 1D kontejner, složený z prvků stejného typu.](images/mvvd_vector.svg)
...
8.  
    ...
    f)  Vyřešte s pomocí funkce `solve()` soustavu lineárních rovnic: $$\begin{array}
          -3x & +2y& +z&=1\\
          -2x & -y& +z&=2\\
          2x & +y& -4z&=0
          \end{array}
...
Za **různorodé** struktury se označují ty, které mohou uchovávat dva a více prvků rozdílného typu současně. Z těch základních to jsou `data.frame` a `list`, dále pak `S4`, nebo `R6` třídy, případně další uživatelem vytvořené struktury.
...
### Podmínky a přepínače

Oběma konstrukty vytváříme v kódu větvení. V R je třeba aby proměnné definující `switch()` nebyla
číselná. `if()` se staví za pomocí relačních operátorů a míří na vyhodnocení `TRUE/FALSE`.  

#### `if()`

Nejjednodušší, ale ne nejbezpečnější, příklad bez dodefinování druhé větve.  

::: {.svg-scale}
```{=html}
{{< include "images/mvvd_if1.svg" >}}
```
:::
...
opts1 <- c("Počet sloupců i řádků musí být shodný", 
           answer = "Proměnná musí mít uvedenou hodnotu v každém řádku", 
           "Všechny proměnné musí být stejného typu")
opts2 <- c("Počet sloupců i řádků musí být shodný", 
           "Proměnná musí mít uvedenou hodnotu v každém řádku", 
           "Všechny proměnné musí být stejného typu", 
...
4.  Vyberte, platné tvrzení pro datovou strukturu list: `r longmcq(opts = opts2)`
5.  Napište výraz pro nepřiřazenou *celočíselnou* hodnotu : `r fitb(answer = "NA_integer_")`\
...
7.  Vytvořte funkci `FD0(x)` s parameterm `x`, který bude reprezentovat teplotu. Funkce `FD0(x)` bude vracet počet hodnot nižších než 0.
:::
