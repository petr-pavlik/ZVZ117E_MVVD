...
2.  
    ...
    a)  V České republice je definován hydrologický rok od 1. 11. do 31. 10. následujícího roku. Přidejte k datové sadě sloupec "h_year" s pomocí funkce `ifelse()`.
    b)  aa
:::
...
4.  Upravte metodu 30denních klouzavých minim pro výpočet základního odtoku tak, aby bylo možné při volání funkce velikost okna upravovat.
:::
...
polygon(x = c(x_vals, rev(x_vals)),             # x coordinates go forward and then backward
  y = c(y_vals, rep(0, length(y_vals))), col = "#1D2D44", border = NA)

```

## Budykova křivka

Na budykovu křivku jsme narazili už při tvorbě grafického výstupu v @sec-grafy. Popisuje referenční klimatické podmínky povodí.
...
1.  Nejprve dvakrát aplikujeme funkci `aggregate()`, abychom dostali součty odtoků za jednotlivé roky a za jednotlivé měsíce v letech.
2.  Výsledné data.frame poté sloučíme do jednoho s pomocí funkce `merge()`, za příponu zvolíme časové okno, ze kterého jsme hodnoty počítali.
...
7.  Čegodajevova rovnice není jediná, které je v praxi používána. Spočítejte pravděpodobnost $P$ těmito dvěma dalšími způsoby a rozdíly diskutujte. 

$$
\begin{align}
P &=\dfrac{m-0.5}{n}\qquad\qquad&\mathrm{Hazen}\\
P &=\dfrac{m-0.44}{n+0.12}\qquad\qquad&\mathrm{Gringorten}\\
\end{align}
$$
...
::: callout-tip
## Úloha
8.  Nahrajte do prostředí data ze souboru. 
9.  Spočítejte:
    a.  průměrný roční minimální průtok.
    b.  počet dnů s denní minimální teplotou $< 20\:^\circ\mathrm{C}$.
    c.  měsíce, kdy je denní teplotní rozsah více než $> 20\:^\circ\mathrm{C}$
    d.  počet dní, kdy byla srážka vyšší než $20\: \mathrm{mm}$ 
:::
