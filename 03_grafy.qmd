...
1.  
    ...
    a)  Zadejte do konzole `demo(graphics)` a projeďte až na konec ukázky. Potom vyčistěte panely **Environment**, **Plots** a **Console**.\
...
2.  
    ...
    a)  Podívejte se na oba krajní grafy předchozího výstupu a popište, rozdíl mezi histogramem a sloupcovým grafem.
    b)  Podívejte se do nápovědy k funkci `hist()` a doplňte následující kód tak, aby koše měly hranice od $-4$ do $4$ s krokem $0,5$. `hist(dfr$Y, breaks =` `r fitb("seq(from = -4, to = 4, by = 0.5)")` `)`
:::
...
1.  Funkce má věšinu svých argumentů parametrizovaných v přednastavenými hodnotami, nebo hodnotami novodvozenými od poskytnutých argumentů. Pro lepší představu o parametrech se podívejte na `plot.default()`.
...
Barvy je možné aplikovat na všechny prvrky grafu. Buď je možné psát barvy dle názvů, například `"white"`, `"orange"` použít funkci `rgb()` a nebo hexadecimální zápis `"#FFF000"`. Výpis všech předdefinovaných barev[^03_grafy-1] lze získat příkazem `colors()`.
...
Správná volba barev v grafech odborných publikací je velmi důležitá. V ideálním případě by grafické škály měly být stejné čitelné jak pro lidi s poruchou barevného spektra, tak v černobílem tisku. Obojí je řešeno například pomocí balíčku `scico` @pedersen2021package. Ve zkratce je problematika vysvětlena například ve [vignette](https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html) balíčku `viridis`.
:::

::: callout-tip
## Úloha

3.  V následujícím příkladu vytvoříte graf **Budykovy křivky**.\
    Postup:\
    ...
    f)  S pomocí následujícího příkazu vytvořte barevné schéma `paleta <- rev(rainbow(20, start = 0, end = 0.6))`
    g)  Vyneste body `x = budyko_dfr$PET_over_P` a `y = budyko_dfr$ET_over_P`.
    h)  Nastavte `xlab = "Ep/P"`, `ylab = "P/Er"`, `xlim` v rozsahu $\phi$ a `ylim` od $0$ do $1,2$. \
    i)  Vyneste do plotu liniové segmenty $(0;0)$ -- $(1;1)$ a $(1;1)$ -- $(2;1)$.
    j)  Funkcí `grid()` přidejte tečkovanou mřížku.
...
plot(x = phi, 
     xlab = expression(italic(E)*p/italic(P)), 
     ylab = expression(italic(P)/ETr), 
     xlim = c(0, 2), 
     ylim = c(0, 1.2), 
     main = "Budyko curve",
     type = "n", 
...
1.  Zde sdělujeme, že celkově bude pole rozděleno v poměru $3\times 3$ a $4\over9$ (levá dolní část) vyplní 1. graf.
...
V prostředí R jsou dva systémy pro tvorbu grafických výstupů. V tomto textu se cíleně držíme postupů, které je možné aplikovat bez instalace jakýchkoliv dodatečných balíčků. Je zde ale
vhodné zmínit, že k funkcím `plot()`, `layout()`, `image()` existují alternativy v balíčcích `ggplot2`, `cowplot`, `rgl` a dalších. 
...
5.  
    ...
    a)  Vytvořte poslouponost `t <- seq(from = 2010, to = 2020, length = 200))` a potom do stejného grafu vyneste $\sin(2\pi(t - 1))$ a $\cos^2(2\pi t)$.
